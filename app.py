...
def generar_analisis_ia(valores: dict, nivel: str, cluster) -> str:
    cliente = obtener_cliente_groq()
    if cliente is None:
        return "No se configuró GROQ_API_KEY en los secrets de la app."

    prompt = f"""Eres un analista actuarial. Datos del individuo:
- Edad: {valores.get('age')}
- BMI: {valores.get('bmi')}
- Hijos: {valores.get('children')}
- Fumador: {valores.get('smoker')}
- Region: {valores.get('region')}
- Sexo: {valores.get('sex')}
- Cargos medicos: {valores.get('charges')}

El modelo lo asigno al cluster {cluster}, nivel de riesgo: {nivel}.

Redacta en espanol, con subtitulos en negrita:
1. "Resumen del resultado": por que probablemente tiene ese nivel de riesgo.
2. "Recomendaciones prudentes": 3 recomendaciones concretas."""
...
if enviar:
    try:
        ...
        st.write(f"Cluster asignado: {prediccion}")

        with st.spinner("Generando análisis..."):
            analisis = generar_analisis_ia(valores, nivel, prediccion)
        st.info(analisis)
